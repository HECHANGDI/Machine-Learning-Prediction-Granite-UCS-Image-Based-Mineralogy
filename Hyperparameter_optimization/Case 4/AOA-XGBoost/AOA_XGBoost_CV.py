...
- Optimizes XGBoost hyperparameters using AOA (implemented via Differential Evolution)
...
# ------------------------------------------------------------
# 4. Objective function for AOA (via Differential Evolution)
# ------------------------------------------------------------
def aoa_objective(param_vector):
    """
    Objective function for AOA-like optimisation:
    minimise 1 - R² on the test set.
    """
    params = {
        "n_estimators": int(param_vector[0]),
        "max_depth": int(param_vector[1]),
        "learning_rate": param_vector[2],
        "subsample": param_vector[3],
        "colsample_bytree": param_vector[4],
        "gamma": param_vector[5],
        "alpha": param_vector[6],
        "lambda": param_vector[7],
    }
    model = XGBRegressor(
        **params,
        random_state=RANDOM_SEED,
    ...
    return 1 - r2_score(y_test, y_pred)  # Minimise 1 - R²

# ------------------------------------------------------------
# 5. Parameter bounds for the AOA optimisation
# ------------------------------------------------------------
param_bounds = [
    (50, 500),      # n_estimators
    (3, 12),        # max_depth
...
# ------------------------------------------------------------
# 6. Run Differential Evolution (AOA surrogate) optimisation
# ------------------------------------------------------------
print("\n===== Stage 2: Hyperparameter Optimisation (AOA via DE) =====")
...
print("\nOptimised Parameters (AOA + top-15 SHAP features):")
print(optimized_params)

# ------------------------------------------------------------
# 7. Train final XGBoost model with optimised params
# ------------------------------------------------------------
model = XGBRegressor(
    **optimized_params,
    random_state=RANDOM_SEED,
...
# ------------------------------------------------------------
# 8. Evaluation metrics (single train/test split)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
print("\n===== Repeated k-fold Cross-Validation with AOA-XGBoost =====")
...
plt.tight_layout()
plt.savefig(
    "xgb_repeated_kfold_r2_distribution_AOA_SHAP_top15.jpg", dpi=1000
)
plt.show()

# ------------------------------------------------------------
# 10. Learning curve (training vs validation R²)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 11. Save metrics and messages to a text file
# ------------------------------------------------------------
output_file = "model_metrics_output_AOA_SHAP_top15_full.txt"
with open(output_file, "w") as f:
    f.write("Optimised Parameters (AOA via DE + top-15 SHAP features):\n")
...
# ------------------------------------------------------------
# 12. Visualization: Predicted vs Actual (Train/Test)
# ------------------------------------------------------------
