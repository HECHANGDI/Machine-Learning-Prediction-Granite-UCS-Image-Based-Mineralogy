...
- Optimizes XGBoost hyperparameters using CGO
...
shap_df = pd.DataFrame(
    {"Feature": X_full.columns, "MeanAbsSHAP": mean_abs_shap}
)

# Rank features by importance
shap_df = shap_df.sort_values("MeanAbsSHAP", ascending=False).reset_index(
    drop=True
)
...
# Precompute the test data for faster evaluation inside objective
...
# ------------------------------------------------------------
# 5. Objective function for CGO (uses SHAP-selected features)
# ------------------------------------------------------------
def cgo_objective(params_vector):
    """
    Objective function for CGO: minimise 1 - R² on the test set.
    """
    params = {
        "n_estimators": int(params_vector[0]),
        "max_depth": int(params_vector[1]),
        "learning_rate": params_vector[2],
        "subsample": params_vector[3],
        "colsample_bytree": params_vector[4],
        "gamma": params_vector[5],
        "alpha": params_vector[6],
        "lambda": params_vector[7],
    }
    model = XGBRegressor(
        **params,
        random_state=RANDOM_SEED,
    ...
    return 1 - r2_score(y_test_np, y_pred)  # Minimise 1 - R²

# ------------------------------------------------------------
# 6. Parameter bounds for CGO optimisation
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 7. CGO implementation
# ------------------------------------------------------------
def cgo_optimize(objective, bounds, iterations=8000, population_size=20):
    """
    Cluster-based Global Optimisation (CGO) for hyperparameter tuning.
    """
    dim = bounds.shape[0]
    # Initialise population uniformly within bounds
    population = np.random.uniform(
        bounds[:, 0], bounds[:, 1], size=(population_size, dim)
    )
    ...
    for iteration in range(1, iterations + 1):
        ...
        scores = Parallel(n_jobs=-1)(
            delayed(objective)(population[i]) for i in range(population_size)
        )
        ...
        # Update the position of candidates
        center = population.mean(axis=0)
        r1, r2 = np.random.rand(2)
        population += r1 * (center - population) + r2 * (best_solution - population)

        # Ensure candidates stay within bounds
...
# ------------------------------------------------------------
# 8. Run CGO optimisation
# ------------------------------------------------------------
print("\n===== Stage 2: Hyperparameter Optimisation (CGO) =====")

best_params_raw = cgo_optimize(
    cgo_objective,
    param_bounds,
    iterations=8000,
    population_size=20,
...
print("\nOptimised Parameters (CGO + top-15 SHAP features):")
print(optimized_params)

# ------------------------------------------------------------
# 9. Train final XGBoost model with optimised params
# ------------------------------------------------------------
model = XGBRegressor(
    **optimized_params,
    random_state=RANDOM_SEED,
...
# ------------------------------------------------------------
# 10. Single train/test split metrics
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 11. Repeated k-fold cross-validation for robustness
# ------------------------------------------------------------
print("\n===== Repeated k-fold Cross-Validation with CGO-XGBoost =====")
...
plt.tight_layout()
plt.savefig(
    "xgb_repeated_kfold_r2_distribution_CGO_SHAP_top15.jpg", dpi=1000
)
plt.show()
...
# ------------------------------------------------------------
# 13. Save metrics and messages to a text file
# ------------------------------------------------------------
output_file = "model_metrics_output_CGO_SHAP_top15_full.txt"
with open(output_file, "w") as f:
    f.write("Optimised Parameters (CGO + top-15 SHAP features):\n")
...
# ------------------------------------------------------------
# 14. Visualisation: Predicted vs Actual (Train/Test)
# ------------------------------------------------------------
