...
- Optimizes XGBoost hyperparameters using Harris Hawks Optimisation (HHO)
...
# ------------------------------------------------------------
# 5. Objective function for HHO (uses SHAP-selected features)
# ------------------------------------------------------------
def hho_objective(params):
    """Objective function for HHO: minimise 1 - R² on the test set."""
    params_dict = {
...
# ------------------------------------------------------------
# 7. Harris Hawks Optimisation (simplified)
# ------------------------------------------------------------
def hho_optimize(objective, bounds, iterations=8000, population_size=20):
    dim = bounds.shape[0]
    ...
    for iteration in range(1, iterations + 1):
        scores = Parallel(n_jobs=-1)(
            delayed(objective)(candidate) for candidate in population
        )
        ...
        # Update the position of hawks (simplified HHO update rule)
        for i in range(population_size):
            for j in range(dim):
                r1, r2 = np.random.rand(), np.random.rand()
                jump_strength = 2 * r1 - 1
                population[i, j] += (
                    jump_strength
...
# ------------------------------------------------------------
# 8. Run HHO optimisation
# ------------------------------------------------------------
best_params_raw = hho_optimize(
    hho_objective,
    param_bounds,
    iterations=8000,
    population_size=20
...
print("\nOptimised Parameters (HHO + top-15 SHAP features):")
...
plt.tight_layout()
plt.savefig("xgb_hho_SHAP_top15_repeated_kfold_r2_distribution.jpg", dpi=1000)
plt.show()
...
# ------------------------------------------------------------
output_file = "model_metrics_output_HHO_SHAP_top15_withCV.txt"
with open(output_file, "w") as f:
    f.write("Optimised Parameters (HHO + top-15 SHAP features):\n")
