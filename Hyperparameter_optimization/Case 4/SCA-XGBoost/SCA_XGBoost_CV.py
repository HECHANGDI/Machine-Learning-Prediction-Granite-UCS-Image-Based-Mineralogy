...
- Optimizes XGBoost hyperparameters using SCA
...
# ------------------------------------------------------------
# 4. Objective function for SCA (uses SHAP-selected features)
# ------------------------------------------------------------
def sca_objective(params):
    """
    Objective function for SCA: minimise 1 - R² on the test set.
    """
    params_dict = {
...
# ------------------------------------------------------------
def sca_optimize(objective, bounds, iterations=8000, population_size=20):
    """
    Sine Cosine Algorithm (SCA) for hyperparameter optimisation.
    """
    dim = bounds.shape[0]
    ...
    for t in range(1, iterations + 1):
        ...
        for i in range(population_size):
            ...
            a = 2 - 2 * (t / iterations)  # linearly decreasing coefficient

            if r1 < 0.5:
                # Move towards best_solution using sine
                step = a * np.sin(2 * np.pi * r2) * (best_solution - population[i])
            else:
                # Move towards best_solution using cosine
                step = a * np.cos(2 * np.pi * r2) * (best_solution - population[i])

            # Optional random interaction with another solution
            if r3 < 0.5:
                random_index = np.random.randint(0, population_size)
                random_solution = population[random_index]
                step += a * r4 * (random_solution - population[i])
...
# ------------------------------------------------------------
# 7. Run SCA optimisation
# ------------------------------------------------------------
best_params_raw = sca_optimize(
    sca_objective, param_bounds, iterations=8000, population_size=20
)
...
print("\nOptimised Parameters (SCA + top-15 SHAP features):")
...
# ------------------------------------------------------------
print("\n===== Repeated k-fold Cross-Validation with SCA-XGBoost =====")
...
plt.tight_layout()
plt.savefig(
    "xgb_repeated_kfold_r2_distribution_SCA_SHAP_top15.jpg", dpi=1000
)
plt.show()
...
# ------------------------------------------------------------
output_file = "model_metrics_output_SCA_SHAP_top15_full.txt"
with open(output_file, "w") as f:
    f.write("Optimised Parameters (SCA + top-15 SHAP features):\n")
