...
- Optimizes XGBoost hyperparameters using SSO (Sparrow Search Optimizer)
...
# ------------------------------------------------------------
# 4. Objective function for SSO (uses SHAP-selected features)
# ------------------------------------------------------------
def sso_objective(params):
    """
    Objective function for SSO: minimise 1 - R² on the test set.
    """
    params_dict = {
...
# ------------------------------------------------------------
def sso_optimize(objective, bounds, iterations=8000, population_size=20):
    """
    Sparrow Search Optimizer (simplified) for hyperparameter optimisation.
    """
    dim = bounds.shape[0]
    ...
    for t in range(1, iterations + 1):
        ...
        # Update the position of sparrows
        ...
        # Ensure sparrows stay within bounds
...
# ------------------------------------------------------------
# 7. Run SSO optimisation
# ------------------------------------------------------------
best_params_raw = sso_optimize(
    sso_objective, param_bounds, iterations=8000, population_size=20
)
...
print("\nOptimised Parameters (SSO + top-15 SHAP features):")
...
# ------------------------------------------------------------
print("\n===== Repeated k-fold Cross-Validation with SSO-XGBoost =====")
...
plt.tight_layout()
plt.savefig(
    "xgb_repeated_kfold_r2_distribution_SSO_SHAP_top15.jpg", dpi=1000
)
plt.show()
...
# ------------------------------------------------------------
output_file = "model_metrics_output_SSO_SHAP_top15_full.txt"
with open(output_file, "w") as f:
    f.write("Optimised Parameters (SSO + top-15 SHAP features):\n")
