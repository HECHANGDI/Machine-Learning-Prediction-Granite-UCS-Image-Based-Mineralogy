...
- Optimizes XGBoost hyperparameters using Multi-Verse Optimisation (MVO)
...
# ------------------------------------------------------------
# 4. Objective function for MVO (uses SHAP-selected features)
# ------------------------------------------------------------
def mvo_objective(params):
    """
    Objective function for MVO: minimise 1 - R² on the test set.
    """
    params_dict = {
...
# ------------------------------------------------------------
# 6. Multi-Verse Optimisation (MVO) implementation
# ------------------------------------------------------------
def mvo_optimize(objective, bounds, iterations=8000, population_size=20):
    """
    Simplified Multi-Verse Optimiser (MVO) for hyperparameter optimisation.
    """
    dim = bounds.shape[0]
    ...
    for t in range(1, iterations + 1):
        ...
        # Weight decreasing over iterations
        W = np.exp(-t / iterations)  # ~1 -> ~0
        ...
        for i in range(population_size):
            r = np.random.uniform()
            if r < 0.5:
                # Explore around the centre
                population[i] = center + W * np.random.uniform(
                    -1, 1, dim
                )
            else:
                # Exploit around the best solution
                population[i] += (
                    W
...
# ------------------------------------------------------------
# 7. Run MVO optimisation
# ------------------------------------------------------------
best_params_raw = mvo_optimize(
    mvo_objective, param_bounds, iterations=8000, population_size=20
)
...
print("\nOptimised Parameters (MVO + top-15 SHAP features):")
...
# ------------------------------------------------------------
print("\n===== Repeated k-fold Cross-Validation with MVO-XGBoost =====")
...
plt.tight_layout()
plt.savefig(
    "xgb_repeated_kfold_r2_distribution_MVO_SHAP_top15.jpg", dpi=1000
)
plt.show()
...
# ------------------------------------------------------------
output_file = "model_metrics_output_MVO_SHAP_top15_full.txt"
with open(output_file, "w") as f:
    f.write("Optimised Parameters (MVO + top-15 SHAP features):\n")
