...
- Optimizes XGBoost hyperparameters using PSO （Particle Swarm Optimisation）
...
# ------------------------------------------------------------
# 4. Objective function for PSO (uses SHAP-selected features)
# ------------------------------------------------------------
def pso_objective(params):
    """
    Objective function for PSO: minimise 1 - R² on the test set.
    """
    params_dict = {
...
# ------------------------------------------------------------
# 6. PSO implementation
# ------------------------------------------------------------
def pso_optimize(objective, bounds, iterations=8000, population_size=20):
    """
    Particle Swarm Optimisation (PSO) for hyperparameter search.
    """
    dim = bounds.shape[0]
    ...
    for t in range(1, iterations + 1):
        ...
        # Progress print
        print(f"Iteration {t}: Best Score = {global_best_score:.6f}")

    return global_best

# ------------------------------------------------------------
# 7. Run PSO optimisation
# ------------------------------------------------------------
best_params_raw = pso_optimize(
    pso_objective, param_bounds, iterations=8000, population_size=20
)
...
print("\nOptimised Parameters (PSO + top-15 SHAP features):")
...
# ------------------------------------------------------------
print("\n===== Repeated k-fold Cross-Validation with PSO-XGBoost =====")
...
plt.tight_layout()
plt.savefig(
    "xgb_repeated_kfold_r2_distribution_PSO_SHAP_top15.jpg", dpi=1000
)
plt.show()
...
# ------------------------------------------------------------
output_file = "model_metrics_output_PSO_SHAP_top15_full.txt"
with open(output_file, "w") as f:
    f.write("Optimised Parameters (PSO + top-15 SHAP features):\n")
