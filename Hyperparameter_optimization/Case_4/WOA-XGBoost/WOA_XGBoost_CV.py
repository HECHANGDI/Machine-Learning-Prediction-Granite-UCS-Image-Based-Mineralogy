...
- Optimizes XGBoost hyperparameters using WOA (Whale Optimization Algorithm)
...
# ------------------------------------------------------------
# 4. Objective function for WOA (uses SHAP-selected features)
# ------------------------------------------------------------
def woa_objective(params):
    """
    Objective function for WOA: minimise 1 - R² on the test set.
    """
    params_dict = {
...
# ------------------------------------------------------------
# 6. WOA implementation
# ------------------------------------------------------------
def woa_optimize(objective, bounds, iterations=8000, population_size=20):
    """
    Whale Optimization Algorithm (simplified) for hyperparameter optimisation.
    """
    dim = bounds.shape[0]
    ...
    for t in range(1, iterations + 1):
        ...
        scores = Parallel(n_jobs=-1)(
            delayed(objective)(individual) for individual in population
        )
        ...
        # Update the position of whales
        for i in range(population_size):
            a = 2 - 2 * (t / iterations)  # Linearly decreasing factor
            r = np.random.rand()
            A = 2 * a * r - a
            C = 2 * np.random.rand()
            p = np.random.rand()
            ...
            if p < 0.5:
                if abs(A) < 1:
                    ...
                    D = np.abs(C * random_solution - population[i])
                    population[i] = random_solution - A * D
            else:
                # Spiral updating position
                D = np.abs(best_solution - population[i])
                b = 1  # constant for spiral shape
                l = (2 * np.random.rand()) - 1  # random number in [-1, 1]
                population[i] = (
                    D * np.exp(b * l) * np.cos(2 * np.pi * l) + best_solution
                )

        # Ensure whales stay within bounds
...
# ------------------------------------------------------------
# 7. Run WOA optimisation
# ------------------------------------------------------------
best_params_raw = woa_optimize(
    woa_objective, param_bounds, iterations=8000, population_size=20
)
...
print("\nOptimised Parameters (WOA + top-15 SHAP features):")
print(optimized_params)

# ------------------------------------------------------------
# 8. Train final XGBoost model with optimised params
# ------------------------------------------------------------
model = XGBRegressor(
    **optimized_params,
    random_state=RANDOM_SEED,
...
# ------------------------------------------------------------
print("\n===== Repeated k-fold Cross-Validation with WOA-XGBoost =====")
...
plt.tight_layout()
plt.savefig(
    "xgb_repeated_kfold_r2_distribution_WOA_SHAP_top15.jpg", dpi=1000
)
plt.show()
...
# ------------------------------------------------------------
output_file = "model_metrics_output_WOA_SHAP_top15_full.txt"
with open(output_file, "w") as f:
    f.write("Optimised Parameters (WOA + top-15 SHAP features):\n")
